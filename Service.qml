...
Item {
  ...
  property int controllerActivityReports: 0
  property string lastControllerActivity: ""

  function controllerActivity() {
    if (!root.idleEnabled) return
    root.controllerActivityReports += 1
    root.lastControllerActivity = nowIso()
    controllerQuietTimer.restart()
    if (root.idledThisCycle) {
      var hadScreensaver = root.screensaverStartedThisCycle
      cancelIdleCycle("controller-activity")
      if (hadScreensaver && !controllerWakeProcess.running)
        controllerWakeProcess.running = true
    }
  }

  // Keyboard/mouse idle AND controller quiet must both reach the normal
  // first timeout. Holding a control counts as use; a resting pad does not.
  Timer {
    id: controllerQuietTimer
    interval: Math.max(1, root.firstIdleTimeoutSeconds * 1000)
    repeat: false
    onTriggered: root.handleIdleChanged()
  }

  Process {
    id: controllerWatcher
    command: ["/usr/bin/python", "-u", Qt.resolvedUrl("gamepad_activity.py").toString().replace("file://", "")]
    running: true
    stdout: SplitParser {
      onRead: data => { if (data.trim() === "activity") root.controllerActivity() }
    }
  }
  ...
  Timer {
    ...
    running: true
    repeat: true
    onTriggered: { if (!controllerWatcher.running) controllerWatcher.running = true }
  }

  Process {
    id: controllerWakeProcess
    command: ["pkill", "-f", "[o]rg.omarchy.screensaver"]
  }
  ...
  function handleIdleChanged() {
    ...
    if (idleMonitor.isIdle) {
      if (!controllerQuietTimer.running) startIdleCycle()
    } else handleActiveSignal()
  }

  function statusJson() {
    return JSON.stringify({
      controllerWatcherRunning: controllerWatcher.running,
      controllerActivityReports: root.controllerActivityReports,
      lastControllerActivity: root.lastControllerActivity,
      controllerQuietCountdownRunning: controllerQuietTimer.running,
